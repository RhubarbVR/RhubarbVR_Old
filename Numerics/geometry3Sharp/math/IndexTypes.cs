...
namespace g3
{
	...
	[MessagePackObject]
	public struct Index2i : IComparable<Index2i>, IEquatable<Index2i>, IConvertible
	{
		[Key(0)]
		public int a;
		[Key(1)]
	...
	public struct Index4i
	{
		...
		public void Sort()
		{
			...
			{ tmp = c; c = b; b = tmp; }
			if (b < a)
			{ tmp = b; b = a; a = tmp; }   // now a is smallest value
			if (b > c)
			{ tmp = c; c = b; b = tmp; }
			if (c > d)
			{ tmp = d; d = c; c = tmp; }   // now d is largest value
			if (b > c)
			{ tmp = c; c = b; b = tmp; }   // bow b,c are sorted
		}
