...
namespace g3
{
	...
	public partial class NTMesh3 : IDeformableMesh
	{
		...
		int replace_tri_vertex(int tID, int vOld, int vNew)
		{
			int i = 3 * tID;
			if (triangles[i] == vOld)
			{ triangles[i] = vNew; return 0; }
			if (triangles[i + 1] == vOld)
			{ triangles[i + 1] = vNew; return 1; }
			if (triangles[i + 2] == vOld)
			{ triangles[i + 2] = vNew; return 2; }
			return -1;
		}
		...
		public virtual MeshResult PokeTriangle(int tid, Vector3d baryCoordinates, out PokeTriangleInfo result)
		{
			...
			add_edge_triangle(eaC, tid);
			add_edge_triangle(eaC, t2);
			add_edge_triangle(ebC, tid);
			add_edge_triangle(ebC, t1);
			add_edge_triangle(ecC, t1);
			add_edge_triangle(ecC, t2);
