...
namespace RhubarbEngine.Components.ImGUI
{
	...
	public class MultiLineSyncObserver : UIWidget, IPropertiesElement
	{
		...
		public MultiLineSyncObserver(IWorldObject _parent, bool newRefIds = true) : base(_parent, newRefIds)
		{

		}
		public MultiLineSyncObserver()
		{
		}
        ...
        public unsafe override void ImguiRender(ImGuiRenderer imGuiRenderer, ImGUICanvas canvas)
        {
            ...
            if (ImGui.InputText((fieldName.Value ?? "null") + $"##{ReferenceID.id}", ref val, (uint)val.Length + 255, ImGuiInputTextFlags.Multiline| ImGuiInputTextFlags.CtrlEnterForNewLine | ImGuiInputTextFlags.EnterReturnsTrue | ImGuiInputTextFlags.AllowTabInput))
			{
				if (target.Target != null)
                {
                    if (target.Target.PrimitiveString != val)
                    {
